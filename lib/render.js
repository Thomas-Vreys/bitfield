...
const getAttr = (e, opt, step, lsbm, msbm) => {
  ...
  const sliceLen = msbm - lsbm + 1;
  const laneStart = opt.index * opt.mod;
  const sliceLsbGlobal = Math.max(e.lsb, laneStart);
  const shift = sliceLsbGlobal - e.lsb;

  if (!Array.isArray(e.attr)) {
    const val = typeof e.attr === 'number' ? (e.attr >> shift) : e.attr;
    return getLabel(val, x, 0, step, sliceLen);
  }
  return e.attr.reduce(
    (prev, a, i) => {
      if (a === undefined || a === null) return prev;
      const val = typeof a === 'number' ? (a >> shift) : a;
      return prev.concat([
        getLabel(val, x, opt.fontsize * i, step, sliceLen),
      ]);
    },
    ['g', {}]);
};
...
const labelArr = (desc, opt) => {
  ...
  desc.map(e => {
    const laneStart = index * mod;
    const laneEnd = (index + 1) * mod - 1;

    // Skip if no overlap
    if (e.msb < laneStart || e.lsb >= laneEnd + 1) {
      return;
    }

    let lsbm = Math.max(0, e.lsb - laneStart);
    let msbm = Math.min(mod - 1, e.msb - laneStart);

    const startsInLane = e.lsb >= laneStart && e.lsb <= laneEnd;
    const endsInLane = e.msb >= laneStart && e.msb <= laneEnd;
    const lsbLabel = (startsInLane ? e.lsb : laneStart) + offset;
    const msbLabel = (endsInLane ? e.msb : laneEnd) + offset;

    if (!compact) {
      bits.push(text(lsbLabel, step * (vflip ? lsbm : (mod - lsbm - 1))));
      if (lsbm !== msbm) {
        bits.push(text(msbLabel, step * (vflip ? msbm : (mod - msbm - 1))));
      }
    }
    if (e.name !== undefined) {
      names.push(getLabel(
